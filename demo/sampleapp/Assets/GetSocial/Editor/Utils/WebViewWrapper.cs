...
{
    public class WebViewWrapper : ScriptableObject
    {
        ...
        static WebViewWrapper()
        {
            _T = typeof(UnityEditor.Editor).Assembly.GetTypes().First(x => x.Name == "WebView");
            ...
            _Unclip = (Func<Rect, Rect>) Delegate.CreateDelegate(typeof(Func<Rect, Rect>), typeof(GUI).Assembly
                .GetTypes()
                .First(x => x.Name == "GUIClip").GetMethod("Unclip", BindingFlags.Static | BindingFlags.Public, null,
        ...
        {
            Invoke(_SetSizeAndPosition, (int) position.x, (int) position.y, (int) position.width,
                (int) position.height);
        }
        ...
        public void DefineScriptObject(string path, ScriptableObject obj)
        {
        ...
        public Action<string> MessageFromJavascriptReceived;
        ...
        protected virtual void OnInitScripting()
        {
            ...
            // register to receive method calls from JS
            DefineScriptObject("window.webScriptObject", this);
        }

        bool ProcessMessage(string jsonRequest, WebViewV8CallbackCSharp callback)
        {
            if (MessageFromJavascriptReceived != null)
                MessageFromJavascriptReceived(jsonRequest);
